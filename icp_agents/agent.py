...
class ICPAgent:
    def __init__(self, capabilities: List, show_planning: bool = False, persona: str = '', config_thread_id: str = ''):
        ...
        system_prompt = """
            ...
            But if it is a task like "I need a campaign about monkeys and it needs to include an image and a video that makes sense for me" your response might be:
            {
    ...
    def create_image(self, prompt: str):
        """ Create an image based on a prompt using ICP tools """
        return create_image_from_prompt(prompt)

    def create_video(self, image_id: str):
        """ Create a video from an image using ICP tools """
        return create_video_from_image(image_id)

    def generate_smart_contract(self, contract_details: Dict):
        """ Generate a smart contract using ICP tools """
        return generate_smart_contract(contract_details)